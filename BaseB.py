  ...
  def model(self, E, E_A, dropout_prob, weights):
    with tf.name_scope('Content_Attention_Module'):
      ...
      with tf.name_scope('sentence_representation'):
        v_ts = tf.transpose(tf.matmul(tf.transpose(E, [0,2,1]), tf.expand_dims(A, 2)), [0,2,1], name='v_ts')                                                               # (S x 1 x d)
        v_ns = tf.identity(v_ts + tf.reshape(v_s, [-1, 1, self.d]), name='v_ns')                                                                                           # (S x 1 x d) 
