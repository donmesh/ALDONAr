...
if __name__ == '__main__':
  ...
  tf.app.flags.DEFINE_string('model_type', 'bert-large-uncased', 'BERT LARGE model')
  ...
  tf.app.flags.DEFINE_boolean('train_model', True, 'Train ALDONAr')
  tf.app.flags.DEFINE_boolean('save_to_files', True, 'Whether to save checkpoints')
  tf.app.flags.DEFINE_boolean('predict_values', True, 'Load and predict with ALDONAr')
  ...
  reader.aspectCategories('Train 2015')
  reader.aspectCategories('Train 2016')
  data_test, longest_sentence_test = reader.readData(FLAGS.test_data_path)
  reader.aspectCategories('Test 2015')
  reader.aspectCategories('Test 2016')
  reader.plotPolarityDistribution(data_train, data_test, print_results=True, plot_results=False)
  ...
  if FLAGS.plot_attention:
    sentence = 'do not get me started on the margaritas either'
    aspect = 'margaritas'
    polarity = 'negative'
    data_attention = np.array([[sentence, aspect, polarity]])
    att, _ = model.predict(data_attention)
    reader.plotAttention(att, sentence.split(), './Results/attention.png')
